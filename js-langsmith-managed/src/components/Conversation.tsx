  ...
  return (
    <>
      ...
      {messages.length === 0 && !stream.isLoading && stream.error ? (
        <div className="error">
          Could not reach the Managed Deep Agent. Check that it's deployed and
          that <code>LANGSMITH_MANAGED_AGENT_ID</code> and{" "}
          <code>LANGSMITH_API_KEY</code> are set, then try again.
        </div>
      ) : null}
    </>
  );
